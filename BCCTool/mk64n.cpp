...
int inf2obj::Convert()
{
	...
	if (file != "")
	{
		...
		if (openFile(f, fsize, file))
		{
			...
			if (fs::exists(fpath + fname + ".nor"))
			{
				nor.readNor(fpath, fname, inf.num_vertices);
				inf.normals = nor.normals;
			}
...
unsigned long fmtNOR::readlong(char*& f, size_t& pos)
{
	unsigned long result = {
		(unsigned int)((uint8_t)f[pos] * 0x00000001) + \
		(unsigned int)((uint8_t)f[pos + 1] * 0x00000100) + \
...
float fmtNOR::readfloat(char*& f, size_t& pos)
{
	float result = 0.0;
...
bool fmtNOR::openFile(char*& f, size_t& fsize, std::string filename)
{
...
bool fmtNOR::readNor(std::string fpath, std::string fname, uint32_t num_ver)
{
	bool sucess = false;
	...
	if (num_ver > 0)
	{
		...
		if (openFile(f1, fsize1, norFile))
		{
			normals = new float[num_ver * 3];
			for (int i = 0; i < num_ver * 3; i++)
			{
				float normal = readfloat(f1, pos1);
...
void fmtINF::write_obj(std::string file)
{
	...
	if (obj.is_open())
	{
		...
		if (normals != NULL) 
		{
			...
			for (int i = 0; i < num_vertices; i++)
			{
				obj << "vn " << std::fixed << std::setprecision(4);
				obj << normals[(i * 3)] << " ";
				obj << normals[(i * 3) + 1] << " ";
		...
		for (int i = 0; i < num_triangles; i++)
		{
			if(i == 0 || tex_triangles[i] != tex_triangles[i - 1])
